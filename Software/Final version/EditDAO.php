...
class EditDAO {
    ...
    public function edit($edit) {
        $sql = "UPDATE Bradford_Memorials SET surname=?, forename=?, regiment=?, Battalion=?, memorial=?, 
    ...
    public function editBiography($editBiography) {
        ...
        $serviceNumber = $editBiography->getServiceNo();
        $error = null;
        ...
        if (!empty($serviceNumber)) {
            ...
            try {
                ...
                if ($result['count'] > 0) {
                    ...
                    error_log("Trying to update Biography_Information where ServiceNo = " . $editBiography->getServiceNo());
    ...
    public function editNewspaperRef($editNewspaperRef) {
        ...
        $sql = "UPDATE Newspaper_ref SET Surname=?, Forename=?, Rank=?, Address=?, Regiment=?, Battalion=?, ArticleComment=?, 
                NewspaperName=?, NewspaperDate=?, PageCol=?, PhotoIncl=? 
        ...
        try {
            ...
            if ($stmt->rowCount() == 0) {
                // Insert a new record
                $sql = "INSERT INTO Newspaper_ref (Surname, Forename, Rank, Address, Regiment, Battalion, ArticleComment, 
                        NewspaperName, NewspaperDate, PageCol, PhotoIncl) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
