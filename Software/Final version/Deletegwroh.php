...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $Cementary = $_POST['Cementary/Memorial'] ?? ''; // Make sure the front-end sends this field name exactly

    // Check for required data
    if (empty($surname) || empty($serviceNo) || empty($Cementary)) {
        echo "Missing required fields for deletion.";
        exit;
    }
    
    // Prepare a DELETE query with placeholders
    $sql = "DELETE FROM gwroh WHERE Surname = ? AND `Service no` = ? AND `Cementary/Memorial` = ?";
    
    if ($stmt = $connect->prepare($sql)) {
        // Bind the values to the query (using sss for three strings)
        $stmt->bind_param("sss", $surname, $serviceNo, $Cementary);
