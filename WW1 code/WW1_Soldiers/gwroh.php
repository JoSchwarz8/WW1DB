...
// Get table column names to debug
$cols_query = "SHOW COLUMNS FROM gwroh";
$cols_result = mysqli_query($connect, $cols_query);
if ($cols_result) {
    $column_names = [];
    while ($col = mysqli_fetch_assoc($cols_result)) {
        $column_names[] = $col['Field'];
    }
    error_log("gwroh table columns: " . json_encode($column_names));
}
?>
...
<script>
    ...
    const serviceNo = row.cells[11].textContent.trim();
    const Cemetery = row.cells[21].textContent.trim();
    
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "Deletegwroh.php", true);
    ...
    editRowBtn.addEventListener('click', () => {
        ...
        // Get data from the selected row to pass to Update-gwroh.php
        const service_no = row.cells[11].textContent.trim(); // Service no column
        const surname = row.cells[1].textContent.trim();
        const forename = row.cells[2].textContent.trim();
        
        // Navigate to Update-gwroh.php with the service number as a parameter
        window.location.href = `Update-gwroh.php?service_no=${encodeURIComponent(service_no)}&surname=${encodeURIComponent(surname)}&forename=${encodeURIComponent(forename)}`;
    });
...
<script>
    // Set up some global variables for batch loading.
    let offset = 0;
    const batchSize = 100; // Adjust batch size as needed.
    let loading = false;
    const tableBody = document.querySelector('#dataTable tbody');
    
    // Function to load a batch of rows from fetchGwroh.php
    function loadBatch() {
        if (loading) return;
        loading = true;
        const xhr = new XMLHttpRequest();
        xhr.open("GET", "fetchGwroh.php?offset=" + offset + "&limit=" + batchSize, true);
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                if (rows.length > 0) {
                    for (let row of rows) {
                        ...
                        // Add a radio button cell first.
                        const radioCell = document.createElement("td");
                        radioCell.innerHTML = '<input type="radio" name="recordSelect">';
                        tr.appendChild(radioCell);
                        
                        // Build table cells for each column; adjust the order to match your table.
                        const fields = [
                    ...
                    // Optionally, if you want to load more batches automatically:
                    // setTimeout(loadBatch, 500); // load next batch after 500ms
                } else {
    ...
    // Option: Use a button to trigger loading more rows:
    // document.getElementById('loadMoreBtn').addEventListener('click', loadBatch);
    
    // For pagination of display (if you also want user-driven navigation among batches already loaded)
    // you can implement that here based on your design.
    
    // Optionally implement infinite scrolling:
    window.addEventListener("scroll", function() {
        // If user has scrolled near the bottom, load the next batch.
